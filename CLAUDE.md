...
- **Squash settings discard everything but the PR title.** This repo's squash-merge config is `squash_title: PR_TITLE`, `squash_message: BLANK` — commit messages and PR body text never reach `main`'s history (the one exception GitHub preserves is `Co-authored-by:` trailers). So: closing keywords (`Closes #N`) go in the **PR description**, not commit messages, or they're silently lost on squash. And Claude session URLs never go in commits or PR bodies — this repo is public; put them nowhere version-controlled or GitHub-hosted-permanently.
